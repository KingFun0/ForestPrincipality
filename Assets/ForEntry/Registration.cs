...
public class Registration : MonoBehaviour
{
    ...
    {
        string insertQuery = "INSERT INTO lal (login, Tree, Gold, Lapis, Ruby, Coins) VALUES (@login, @Tree, @Gold, @Lapis, @Ruby, @Coins)";
        string login = loginInputField.text;
        using (SqliteCommand cmd = new SqliteCommand(insertQuery, dbConnection))
        {
            cmd.Parameters.AddWithValue("@login", login);
            cmd.Parameters.AddWithValue("@Tree", 0);
            cmd.Parameters.AddWithValue("@Gold", 0);
            cmd.Parameters.AddWithValue("@Lapis", 0);
            cmd.Parameters.AddWithValue("@Ruby", 0);
    ...
    private void RegisterUser()
    {
        ...
        PlayerResources.loginPlayer = login;
    }
